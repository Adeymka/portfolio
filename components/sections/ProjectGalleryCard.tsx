...
export default function ProjectGalleryCard({
  ...
  const hasLink = Boolean(caseStudyUrl?.trim());
  ...
  const content = (
    <>
      ...
      <div className="relative overflow-hidden bg-fb-gray aspect-video">
        {project.image ? (
          <Image
            src={project.image}
            alt={project.title}
        ...
        {/* Coming soon badge — when no slug */}
        {!hasLink && (
          <span className="absolute right-2 top-2 rounded-full bg-fb-gray px-2 py-0.5 text-xs text-fb-text-secondary">
            Coming soon
          </span>
        )}
