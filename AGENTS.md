...
- Stack : Next.js 14 App Router, Tailwind, Supabase, Framer Motion ; admin sous `/admin`, protégé par auth Supabase (middleware), front-office sous `app/(main)`.
- Données : projets, skills, messages, certifications, `site_stats` en Supabase ; migrations avec `npm run db:migrate` (CLI en devDependency) ; `site_stats` n’expose plus que les années d’expérience (clé `years_experience`).
...
- Stats affichées (profil, contact, page Projets) : nombre de projets depuis la table `projects`, années d’expérience depuis `site_stats.years_experience` ; récupération robuste dans `lib/supabase/queries.ts` (`getSiteStats`).
- UI côté visiteur : ne pas afficher de métrique « clients satisfaits », ni de boutons d’édition (ex. « Edit cover ») ou de faux badges de notification/dates de disponibilité ; le portfolio doit se comporter comme un site vitrine simple.
